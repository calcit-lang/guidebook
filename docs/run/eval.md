...
```bash
$ calcit eval 'echo |demo'
1
took 0.07ms: nil
```

```bash
$ calcit eval 'echo "|spaced string demo"'
spaced string demo
took 0.074ms: nil
```
...
calcit demos/calcit.cirru eval 'range 3'
# Output: ([] 0 1 2)

# Use let bindings
calcit demos/calcit.cirru eval 'let ((x 1)) (+ x 2)'
# Output: 3
```
...
```bash
calcit demos/calcit.cirru eval --dep ~/.config/calcit/modules/respo.calcit/ -- 'ns app.demo $ :require respo.util.detect :refer $ element?\n\nelement? nil'
```
...
calcit demos/calcit.cirru eval 'let ((x 1)) (assert-type x :string) x'
# Error: Type mismatch...

# Correct type passes
calcit demos/calcit.cirru eval 'let ((x 1)) (assert-type x :number) x'
# Output: 1
```
...
```bash
calcit eval '&list:nth ([] :a :b :c) 1'
# Output: :b

calcit eval '&map:get ({} (:x 1) (:y 2)) :x'
# Output: 1
```
...
calcit eval 'type-of range'
# Output: :fn

# Test with sample data
calcit eval '-> (range 5) (map inc) (filter (fn (x) (> x 2)))'
# Output: ([] 3 4 5)
```
...
- **No extra brackets**: Cirru syntax doesn't need outer parentheses at top level
  - ✅ `calcit eval 'range 3'`
  - ❌ `calcit eval '(range 3)'` (adds extra nesting)
...
- Use `calcit cirru parse-oneliner` to debug parse issues

### Query Examples

Use `calcit query examples` to see usage examples:

```bash
calcit demos/calcit.cirru query examples calcit.core/let
calcit demos/calcit.cirru query examples calcit.core/defn
```

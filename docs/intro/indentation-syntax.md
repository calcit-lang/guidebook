...
Calcit source uses tools from the [Cirru Project](http://cirru.org/). The canonical `calcit.cirru` snapshot stores the program as Cirru EDN with quoted Calcit syntax trees, and can be maintained through the current CLI structural workflow or [Calcit Editor](https://github.com/calcit-lang/editor/).
...
For the current source-first workflow, read `calcit docs read upgrade --full`.
